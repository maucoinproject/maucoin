...
#if defined(NDEBUG)
# error "Maucoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x3e3b43139e5d6ff2cb2bc88fb808aeeedf3351b3f6074978a03752941919728c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Maucoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Maucoin Signed Message:\n";
...
{
    // Maucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Maucoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Maucoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Maucoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Maucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Maucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3e3b43139e5d6ff2cb2bc88fb808aeeedf3351b3f6074978a03752941919728c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 30/Aug/2016 ,In West Bank, Israel Quietly Legalizes Pirate Outposts";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1472584042;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1254831;
        ...
        assert(block.hashMerkleRoot == uint256("0xb934745c166d82986a61aafd96637e4a8f2bba655c49495db62f7ebf6c2c75b6"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Maucoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MaucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MaucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MaucoinMiner(CWallet *pwallet)
{
    printf("MaucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("maucoin-miner");
    ...
    try { loop {
        ...
        printf("Running MaucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MaucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MaucoinMiner, pwallet));
}
